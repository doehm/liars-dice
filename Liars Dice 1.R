...
play.liars.dice(auto = FALSE, players = 2, num.dice = 6, verbose = 1, agents = agents)

a <- sapply(1:100, function(x) play.liars.dice(verbose = 0, auto = TRUE, players = 4, num.dice = 6, agents = agents))
...
a <- parSapply(clust, 1:10000, function(x) play.liars.dice(verbose = 0, auto = TRUE, players = 2, num.dice = 6, agents = agents))
stopCluster(clust)
end <- Sys.time()
end-strt
...
table(a)
wins <- table(a)[2]
z <- rbeta(1e5, wins, 1e4-wins)
ggplot(as.data.frame(z), aes(x = z)) + geom_histogram(fill = "darkturquoise", col = "grey20")

# not a huge difference in wins for each agent but it's clear playing the numbers will win 
# the long game by winning more thn 50% of the games
